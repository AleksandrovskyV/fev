...
const canvasControls = document.getElementById("sCanvasControls");
...
const infoButton = document.getElementById("aboutBtn");
...
const gsSLetterLine = document.getElementById("gsSLetterLine"); // При старте
const generateApply = document.getElementById("generateApply");

const gsLetterLine = document.getElementById("gsLetterLine"); // Из Append
...
let dragTarget = null;
let dragOverCanvas = false; // флаг для dropdown - убрано здесь
...
function refreshFromScratch() {
    ...
    gsLetterLine.value = [...new Set(result)].join('');
...
function toggleAndAdd(key, button, charArray) {
    if (!setupFlags[key]) {
        lastInputValue = gsLetterLine.value; 
        gsLetterLine.value += charArray.join('');
    } 
    else {

        if (SMART_CLEANUP) {
            const charsToRemove = new Set(charArray);
            gsLetterLine.value = gsLetterLine.value
                .split('')
                .filter(char => !charsToRemove.has(char))
                .join('');
        } else {
            gsLetterLine.value = lastInputValue;
        }
    }
...
function updateInternalGlyph() {
    ...
    //USER_INPUT_GLYPH = [...new Set(gsLetterLine.value)].join('').trim();
    //USER_INPUT_GLYPH = USER_INPUT_GLYPH.split(' ').join('');
    const user =  [...new Set([...gsSLetterLine.value].filter(char => char !== ' ')),].join('');

    // add remove ! and reback after

    const append = "!"
    USER_INPUT_GLYPH = [...user, ...append];
}
...
function redrawActiveGlyphInCanvas(erender = true) { // Под Editor Canvas
    ...
    if(currentDataArray && currentItemIndex !== null){
        ...
        console.log("redrawActiveGlyphInCanvas:", currentGlyph );
    }else{
...
function renderSampleCanvas() {
    ...
    if(messageSampleCanvas && !startwindow){
        ...
        ctxSample.fillText("DRAG GLYPH HERE...E", cx, cy - 20);
...
window.addEventListener("drop", (e) => { // три кейса: шрифт, внутренний элемент плитки, изображение для canvas
    console.log("DROP");
    e.preventDefault();
    ...
    const isFont = handleFontFile(file);
...
/* 
// вроде можно убрать это не сломает
...
canvasSample.addEventListener("mouseup", e => {
    if(startwindow){
        document.activeElement.blur(); // windowblur
    }
    e.preventDefault();
});
...
function resizeCanvas() {
    // Используйте innerWidth/innerHeight для точности
    canvasSample.width = window.innerWidth;
    canvasSample.height = window.innerHeight;
    renderSampleCanvas();
}
...
function callGlypthEditor(option) {
	...
	if(glyphEditor==true){ // открытие
        ...
        glyphMapBtn.classList.add("active")
	    ...
	    glyphMapBtn.classList.remove("active");
...
function callGlypthMap(option) {
    ...
    console.log("callGlypthMap:", glyphMapViewer);
...
let cPanelDisplay = false;


function callCPanel(option) {
    if (closeOverlays()) return;

    if (option !== undefined) {
        cPanelDisplay = option;
    } else {
        cPanelDisplay = !cPanelDisplay; 
    }
    
    console.log("callCpanel:", cPanelDisplay);
    
    const cPanelRow = document.getElementById("cPanelRow");

    const aboutBtn = document.getElementById("aboutBtn");
    const exportPanelBtn = document.getElementById("exportPanelBtn");
    

    if(cPanelDisplay==true){

        openPanel.classList.add("active")
        
        aboutBtn.hidden = false;
        exportPanelBtn.hidden = false;
        glyphMapBtn.hidden = false;
    }else{
        openPanel.classList.remove("active")

        aboutBtn.hidden = true;
        exportPanelBtn.hidden = true;
        glyphMapBtn.hidden = true;
    }

}

const openPanel = document.getElementById("openPanel");
openPanel.onclick = () => callCPanel();
...
glyphMapBtn.onclick = () => callGlypthMap();
...
window.onkeydown = (e) => {
    ...
    if (key >= "1" && key <= "9") { //Numeration-1
        ...
        if (i == 0)      cPanelDisplay && callReadme();
        else if (i == 1) cPanelDisplay && callPanelExport();
        else if (i == 2) cPanelDisplay && callGlypthMap();
        else if (i == 3) callCPanel();
        else if (i == 7) callCPanel();
