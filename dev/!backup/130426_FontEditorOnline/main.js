...
let pasteMessageActive = true; // Тот самый флаг, который вы переключите в false
...
function createProcedureGlypth() {
  console.log("CreateProcedureGlyph")
...
function updateProcedureGlyph() {
  console.log("updateProcedureGlyph");
  if (currentProcedureGlyph && currentGlyph) {
    const recipeFn = GLYPH_RECIPES[currentGlyph.name];
    if (recipeFn) {
      currentProcedureGlyph.update(currentGlyph.name, recipeFn, GFONT_PARAMS);
    } else {
      console.warn(`Рецепт для ${currentGlyph.name} не найден!`);
    }
  }
}
...
function creteGlyphPromProcedure(symbol, newIndex) {
        ...
        if (caseKey !== null) {
            ...
            for (let i = 0; i < symbolCases[caseKey].length; i++) {
                ...
                if (caseChar === targetSymbol){
                    console.log("Уже разобранный вариант");
            ...
            console.log('case не найден в symbolCases');
            console.log("Reciept Not Found... GenerateBlank");
        }
...
function processGlyphGenerate() {
    updateInternalGlyph();
    
    GENERIC_TABLE = getTable();

    console.log("Финальная карта", GENERIC_TABLE);

    // 1. Сначала создаем .notdef (техническая заглушка)... нужно добавить ещё два спец символа
    const notdefGlyph = new opentype.Glyph({
        name: '.notdef',
        advanceWidth: 500,
        path: new opentype.Path()
    ...
    font = new opentype.Font({
        ...
        tables: {
            ...
            maxp: {
                version: 1.0,
                numGlyphs: [notdefGlyph, ...generatedGlyphs].length
            }
        }
    });
    ...
    //font.tables = font.toTables();
    //console.log(font.tables.maxp.numGlyphs); 

    //font.styleName = GFONT_PARAMS.style;
    //console.log("Check Style and Data", font.styleName, font.designer);
...
// CODEMIRROR START (1459 - 1538)

let EditorView, basicSetup, Decoration, MatchDecorator, ViewPlugin, javascript, Transaction, HighlightStyle, tags, syntaxHighlighting
let moduleCodeLoad = false;
let myTheme = null; 
let myHighlightStyle = null;
let selfPlugin = null;
let selfDecorator = null;

// Extend Online
function waitForCM() {
    if (window.CM) {
        ({ EditorView, basicSetup, Decoration, MatchDecorator, ViewPlugin, javascript, Transaction, HighlightStyle, tags, syntaxHighlighting } = window.CM);

        moduleCodeLoad = true;

        // Создаем декоратор для слова "self"
        selfDecorator = new MatchDecorator({
            regexp: /\bself\b/g,
            decoration: Decoration.mark({ class: "cm-self-word" })
        });

        // Создаем плагин, который будет применять этот декоратор
        selfPlugin = ViewPlugin.fromClass(class {
            constructor(view) { this.decorations = selfDecorator.createDeco(view); }
            update(update) { this.decorations = selfDecorator.updateDeco(update, this.decorations); }
        }, {
            decorations: v => v.decorations
        });

        myTheme = EditorView.theme({
          "&": { color: "#ddd", backgroundColor: "black" },
          ".cm-content": { padding: "10px", fontFamily: "sans-serif", fontSize: "15px", lineHeight: "1.5", position: "relative" },
          ".cm-selectionBackground": { backgroundColor: "rgba(153, 255, 119, 0.5)", position: "absolute", zIndex: 5 },
          "&.cm-focused .cm-selectionBackground": { backgroundColor: "rgba(153, 255, 119, 0.5) !important" },
          ".cm-selectionMatch, .cm-selectionMatch *": { 
            color: "#ddd !important", 
            backgroundColor: "rgba(153, 255, 119, 0.25) !important" 
          },
          ".cm-cursor": { borderLeft: "1px solid #fff", zIndex: 10 },
          ".cm-line.cm-activeLine": { backgroundColor: "transparent", zIndex: 0},
          ".cm-gutters": { display: "none" },
          ".cm-self-word": { color: "#80a143" },
        }, {dark: true});
        ...
        myHighlightStyle = HighlightStyle.define([
          ...
          {
            tag: tags.bool, // true, false
            color: "#c93737",
          },
          {
            tag: tags.null, // null
            color: "#c93737", // можно сделать другим, если нужно
        },
...
function createCodeEditor() {
    if (!moduleCodeLoad) return console.warn("CodeMirror ещё не загрузился");
    ...
    codeEditor = new EditorView({
        doc: trimmedBody, // только тело функции
        extensions: [
            basicSetup,
            javascript(),
            selfPlugin,
            myTheme,
            syntaxHighlighting(myHighlightStyle), // 👈 ВОТ ЭТО ТЫ ПРОПУСТИЛ
            EditorView.updateListener.of(v => {
                if (v.docChanged && v.transactions.some(t => t.isUserEvent)) {
                    // при сохранении оборачиваем обратно
                    const codeString = `(p, self) => {\n${v.state.doc.toString()}\n}`;
                    updateRecipeFromEditor(currentGlyph.name, codeString);
                }
            })
        ],
        parent: editorCodeBlock
    });
...
function updateCodeEditor() {
    ...
    codeEditor.dispatch({
        changes: { from: 0, to: codeEditor.state.doc.length, insert: trimmedBody },
...
function printInformation(variable, fullwrite = true, id = "") {
    ...
    const copyrightSource = getSafeName('copyright');
...
function printGlyphInformation(index) {
    ...
    if (!glyph){
        try {
            ...
            console.log(" Print из glyphArray", aindex);
            glyph = glyphArray[aindex].glyph;
        } catch (err) {
            console.log("Print Вышли из странной ветки");
            return
        }
    }
...
function redrawAllProcedure(erender=true){
    ...
    redrawCurrentGlyphPath(erender);

    const newStyleName = getWeightName(GFONT_PARAMS.t*4 || 400);
    GFONT_PARAMS.style = newStyleName;
    font.styleName = newStyleName;
...
function saveFontlib(font, buffer, originalFileName) {
    ...
    // Пробуем взять имя из метаданных, если нет — используем имя файла
    ...
    // Сохраняем распакованный буфер, чтобы не парсить WOFF2 дважды
    fontlib.set(fontName, {
        buffer: buffer, 
        name: fontName
    });
    
    console.log(`Шрифт "${fontName}" сохранен в библиотеку.`);
}
...
function rollbackFontlib() {
    ...
    // Удаляем текущий, чтобы "откатиться" навсегда
    if (fontlib.size == 1){
...
function updateCMAP(mode=0) {

    // modes: 0 - parseGlyph; 1 - parseComponents 
...
function appendGlyph() {
    ...
    let ucode = oUnicodeInput.value;
    
    const foundName = glyphArray.find(item => item.name === newName);

    if(foundName){
        oCommentLine.hidden = false;
        oCommentLine.innerHTML = `
            <label> Глиф с таким именем уже существует...</label>
        `;
        return
    }
    ...
    if (isNaN(ucode) || ucode < 0 || ucode > 0x10FFFF) {
        oCommentLine.hidden = false;
        oCommentLine.innerHTML = `
            <label> Странный Unicode...</label>
        `;
        console.log("Странный Unicode..");
        return 
    }

    if(generatedFont){ advW = (GFONT_PARAMS.w) || 400;
    }else{ advW = 500;
    }
    ...
    //console.log("appendGlyph R", glyphArray, item);

    createTile(item, newIndex, fragment);
    cmap.appendChild(fragment);
...
function funcPlusCMapBtn(){
    ...
    const oNameInput = document.getElementById("oNameInput");
    console.log(compViewer.current);

    if (compViewer.current !== compViewer.previous) {
        oNameInput.value = "";
        oUnicodeInput.value = "";
    }

    if(!compViewer.current){ // Add Glyph Process (Global Variable)
        ...
        pushBtn.onclick = () => appendGlyph();

    }else{ // Add Component Process

        oNameLabel.textContent = "ComponentName";
...
function createCMapPlusButton() {
    ...
    if (!existingBtn) { // > create
        ...
        const cMapMode = compViewer.current ? "1" : "0";
...
function createTile(item, index, container) {
    ...
    }else if(index == currentGlyphIndex){
        currentGlyph = glyph;
        currentGlyphIndex = index;
        console.log("Обновили текущий");
    }

    tile.onclick = () => clickMap(tile, item.index);
    tile.draggable = true;
    tile.ondragstart = (e) => {
...
*/
    ...
    const newDesigner = getSafeName('designer')+postFixDesigner; 
    ...
    preserve_font = new opentype.Font({
    ...
    console.log("Шрифт Preserved!", preservedFont);
...
function redrawCurrentGlyphPath(erender = true) { // Под Editor Canvas
    ...
    if (!glyph){
        try {
            const aindex = glyphArray.findIndex(it => it.index === currentGlyphIndex);
            console.log("Странная ветка", aindex);
            glyph = glyphArray[aindex].glyph;
        } catch (err) {
            console.log("Вышли из странной ветки");
            return
        }
    }
    ...
    if (generatedFont){
        ...
        if (label === "RSB"){
            if (typeof manualUpdateControlValue === "function") {
                manualUpdateControlValue("aw", resultValue, false); // без рендера
            }
        } 
...
function updateGuidelines(params) {
    ...
    //const ascY = font.ascender ? baseline - font.ascender * scale : null;
    //const capY = (font.tables.os2?.sCapHeight) ? baseline - font.tables.os2.sCapHeight * scale : null;

    // Tempory (Changeble)
    const aVal = fontOverrides.ascender || font.ascender || 0;
    const cVal = fontOverrides.sCapHeight || font.tables.os2?.sCapHeight || 0;
    const xVal = fontOverrides.sxHeight || font.tables.os2?.sxHeight || 0;
    const dVal = fontOverrides.descender || font.descender || 0;
...
function renderEditorCanvas() {
    ...
    //const { renderSize, x, baseline,  width, scale, zoom } = getTransformParams();
...
function commitGlyphEdits(){
    ...
    if(currentProcedureGlyph && pEditMode){
        console.log("CommitChange: Procedure", currentGlyphIndex);

        const data = currentProcedureGlyph.exportData(); 
        newPath = data.path;

        currentGlyph.advanceWidth = data.advanceWidth;
    }

    if(currentContours && !pEditMode){
        if (font.variation) {
            console.log("Шрифт вариативный - не применяем трансформацию")
            return
        }
        console.log("CommitChange: Contour");
        // правильная preview svg плитка \ рендеринг в Canvas
        // но на сохраении windows всё же показывает дырки в пересечениях
    ...
    //currentGlyph.data = { overlap: true };
    //console.log("debugFBooleanFlag", currentGlyph.data.overlap);
    console.log("DebugWinding", newPath);
...
function hitTestControls(mouseX, mouseY){
    const { scale, x, baseline } = getTransformParams()
    const translateX = x
...
canvas.addEventListener("mousedown", e => {
    ...
    const { x: mx, y: my } = getMousePosition(canvas, e);
    const { x: tx, baseline, yCenter, scale } = getTransformParams();
...
canvas.addEventListener("mousemove", e => {
    ...
    const { x: mx, y: my } = getMousePosition(canvas, e);
    const { x, baseline, yCenter, scale} = getTransformParams();
...
canvasSample.addEventListener("mousedown", e => {
    ...
    if (sBackgroundSelected) {
        ...
        if (!e.shiftKey && !selectedObjects.includes(clicked)) {
            ...
            const indexInArray = clicked.item.index; 
            currentGlyphIndex = indexInArray;
    
            //console.log("Клик по глифу с индексом:", indexInArray, currentGlyphIndex);
            redrawCurrentGlyphPath();
            updateCodeEditor();
...
function clickMap(element, index){
    //const item = glyphArray.find(it => it.index === index); 
...
let firstCallGEditor = false;

function checkActiveTileStyle() {
    // SET ACTIVE CLASS TILE
    // SCROLL CMAP?
    if (tempElement === null && currentGlyphIndex !== null) {
        const activeTile = document.querySelector(`#cmap .tile[data-index="${currentGlyphIndex}"]`);
        
        if (activeTile) {
            activeTile.classList.add("active");
            tempElement = activeTile;
        }
    }
}
...
function switchArrayViewer(option, draw=true) {
    if (closeOverlays()) return;
     ...
     if (option !== undefined) {
        compViewer.previous = compViewer.current;
        compViewer.current = option;
    } else {
        compViewer.previous = compViewer.current;
        compViewer.current = !compViewer.current; 
    }

    console.log("callCompViewer:", compViewer.current);
    
    if(compViewer.current==true){
        switchCmapViewer.classList.add("active");
        ...
        switchCmapViewer.classList.remove("active");
    ...
    if (cMapPlusButton) {
        ...
        if (compViewer.current !== compViewer.previous) {
            const beforeChange = compViewer.previous ? "1" : "0";
            const currentValue = compViewer.current ? "1" : "0";
...
function updateExportLines(){
    ...
    const gStyleName = getWeightName(GFONT_PARAMS.t*4 || 400);
...
exportPanelBtn.onclick = () => callPanelExport();
...
function exportFontRebuild() {
    ...
    if(!generatedFont){
        ...
        newStyleName = getSafeName('fontSubfamily') || "Regular";
        newDesigner = getSafeName('designer') || "Unknown";
