...
function hideSelTilesOld(mode, value) { 
    // Метод открытия\закрытия тайлов из Canvas

// mode = "soloClick", "shiftClick", "unhideAll"
// value: name or names, or unicode

    console.log("hideSelTiles", mode, value,"searchBarFlag", searchBarFlag)

    const tiles = document.querySelectorAll('#cmap .tile');
    const allowedNames = Array.isArray(value) ? value : [value];
    
    const allowedUnicodes = Array.isArray(value) ? value : [value];
    const stringUnicodes = allowedUnicodes.map(val => String(val));

    if(mode === "soloClick" && value){ // Click on Element
        
        if(textInSearchBar){  // value = unicode // Текст введён, просто откроем плитку

            // логика: если символа нет в searchBar > покажем отдельно плитку
            // если есть - мы не трогаем search, просто показываем вместе c search блоком

            targetTile = document.querySelector(`#cmap .tile[data-unicode="${value}"]`);

            if (targetTile){

                if(specialTileTemp !== null){
                    const spName = specialTileTemp.dataset.name;
                    const spUni  = specialTileTemp.dataset.unicode;

                    const notInSearch = !searchKeys.includes(spName); //&& !searchKeys.includes(spUni);

                    if (notInSearch) {
                        specialTileTemp.hidden = true;
                    }
                }

                targetTile.hidden = false;
                specialTileTemp = targetTile;
                
                console.log("Найдена плитка:", targetTile.dataset.name);
            } else {
                console.log("Плитка с таким юникодом не найдена");
            }

            return
        }else{ // скрываем все кроме выделенного
            tiles.forEach(tile => {
                const tileName = tile.dataset.name || "";
                const tileUnicode = tile.dataset.unicode;

                //const isMatch = allowedNames.includes(tileName); // old
                const isMatch = stringUnicodes.includes(tileUnicode);
                tile.hidden = !isMatch;
            });
        }
    } 

    if (mode === "unhideAll"){ // Click to Canvas (Expand All)
        
        if(textInSearchBar){ // Убираем только temp плитку
            ...
            tiles.forEach(tile => tile.hidden = false);
            console.log("hideSelTiles Unhide All")
            return;
        }
    }

    if(mode === "shiftClick" && value){ // Click with Shift
        if(textInSearchBar){
            stringUnicodes.forEach(uni => {
                const targetTile = document.querySelector(`#cmap .tile[data-unicode="${uni}"]`);
                if (targetTile) {
...
function updateSearchBar(value){ // в value сейчас приходит только unicode из другой ветки
    ...
    */
    ...
    tiles.forEach(tile => {
        ...
        const isMatch = searchKeys.some(key => key === name);
...
function drawGuidelinesOld(params) {
    ...
    if (font.tables.os2 && font.tables.os2.sxHeight) {
        const sxH = baseline - font.tables.os2.sxHeight * scale;
        drawLineWithLabel(sxH,  "X-Height", "#aa6600");
    }

    drawLineWithLabel(baseline, "Baseline", "#666");
    
    if (font.descender) {
        const descY = baseline - font.descender * scale;
        drawLineWithLabel(descY,  "Descender", "#4444ff");
    }
...
function callGlobalCanvas(option){
    ...
    if(globalCanvasFlag==true){
        ...
        //globalCanvasBtn.classList.add("active")
        if (gCanvasWrapper) gCanvasWrapper.hidden = false;

        //InfoSection.hidden = true;
	}else{

	    //globalCanvasBtn.classList.remove("active")
...
// (old) callGlobalCanvas, callEditorCanvas
//const globalCanvasBtn = document.getElementById("SBOARD");
//const editorCanvasBtn = document.getElementById("GBOARD");
//globalCanvasBtn.onclick = () => callGlobalCanvas();
